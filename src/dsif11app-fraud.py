...
# Define pydantic model for dataframe that will be posted by
# client in form of dictionary
class json_DataFrame(BaseModel):
    index: List[int]
    columns: List[str]
    data: List[List[Any]]
...
@app.post("/uploadfile/")
def upload_file(input_data: json_DataFrame):
    ...
    # Select the features used in loaded_pipeline
    model_features = ['transaction_amount', 'customer_age', 'customer_balance']
    X = transactions[model_features]

    # Make predictions with loaded_pipeline
    transactions['pred_fraud'] = loaded_pipeline.predict(X)
    transactions['pred_proba_fraud'] = loaded_pipeline.predict_proba(X)[:, 1]
    transactions['pred_proba_fraud'] = transactions['pred_proba_fraud'].apply(lambda x: round(x, 5))

    # Return updated Dataframe to the client in form of dictionary
    return {"json":transactions.to_dict('split')}
